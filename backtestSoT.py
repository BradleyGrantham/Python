...
with open('backtest.csv') as csvfile:
	for team in teams:
		...
		csvfile.seek(0)
		for row in reader:
			if datetime.datetime.strptime(row['Date'], "%d/%m/%y") > datetime.datetime(2012, 6, 1) and datetime.datetime.strptime(row['Date'], "%d/%m/%y") < datetime.datetime(2013, 6, 1):
...
with open('backtest.csv') as csvfile:
	...
	for row in reader:
		...
		if datetime.datetime.strptime(row['Date'], "%d/%m/%y") > datetime.datetime(2013, 6, 1) and datetime.datetime.strptime(row['Date'], "%d/%m/%y") < datetime.datetime(2014, 6, 1):
			if row['HomeTeam'] == 'Cardiff' or row['HomeTeam'] == 'Hull' or row['AwayTeam'] == 'Cardiff' or row['AwayTeam'] == 'Hull' or row['HomeTeam'] == 'Crystal Palace' or row['AwayTeam'] == 'Crystal Palace':
				amountBetted+=0
			else:
