...
for year in range(2010, 2015):
	...
	teams = set()
	goodTeams = ['Chelsea', 'Man United', 'Man City']
	xGoals = []
	yGoals = []
	dates = []
	totalHomeGoals = []
			...
			if datetime.datetime.strptime(row['Date'], "%d/%m/%y") > datetime.datetime(year+1, 6, 1) and datetime.datetime.strptime(row['Date'], "%d/%m/%y") < datetime.datetime(year+2, 6, 1):
				...
				if len(homeGoalsDict[homeTeam]) != 0 and len(awayGoalsDict[awayTeam]) != 0 and len(homeGoalsDict[homeTeam]) > 6 and len(awayGoalsDict[awayTeam]) > 6:	
					...
					if 1/drawProb < float(row['B365D']):
						amountOfBets += 1
						amountBet = 5/(float(row['B365D'])-1)
						amountBetted += amountBet
						oddsBet.append(float(row['B365D']))
					...
					if homeTeam in goodTeams and awayTeam in goodTeams:
						errorsquared.append((float(x) - float(row['FTHG']))*(float(x) - float(row['FTHG'])))
						errorsquared.append((float(y) - float(row['FTAG']))*(float(y) - float(row['FTAG'])))
...
print(average(xGoals), average(yGoals))
